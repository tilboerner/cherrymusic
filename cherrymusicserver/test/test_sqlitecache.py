...
class AddFilesToDatabaseTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.Cache.create_and_alter_tables(suppressWarning=True)
    ...
    def test_add_to_file_table(self):
        ...
        dbdef = cherry.db.dbdef(sqlitecache.DBNAME)
        colnames = tuple(p.name for p in dbdef.types['files'])
...
class RemoveFilesFromDatabaseTest(unittest.TestCase):
    ...
    def testRollbackOnException(self):
        ...
        class BoobytrappedConnector(MemConnector):
            ...
            def __init__(self):
                ...
                self.Connection = type(
                    '%s.BoobytrappedConnection' % (self.__class__.__module__),
                    (self.Connection,),
                    {'execute': self.__execute})

            def __execute(self, stmt, *parameters):
                ...
                return super(self.Connection, self._cxn).execute(stmt, *parameters)

        # SPECIAL SETUP
        cherry.db.connector = BoobytrappedConnector()
        self.Cache = sqlitecache.SQLiteCache()
        self.Cache.create_and_alter_tables(suppressWarning=True)
        self.Cache.full_update()
        ...
        # ASSERT
        self.assertLessEqual(1, cherry.db.connector.exceptcount,
